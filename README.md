...
一个基于 Astro 和 GitHub Pages 的个人博客。当前版本采用战术终端 / 档案界面风格：黑白灰高对比、橙色警示点缀、斜切卡片、编号标签和工业网格；内容包含生活档案、技术记录、个人简介、灵感仓库和网页发布后台。
...
本次没有内置或下载任何明日方舟官方素材。首页主视觉预留了可选路径：`public/images/arknights/hero.png`。如果你后续把已授权素材放到这个位置，首页会自动显示；文件不存在时会显示原创几何终端占位，不会产生破图。

## 站点结构

- `/`：首页，包含快速索引面板、生活档案、技术记录、个人简介、终端组件和灵感仓库预览。
- `/life/`：生活档案页面，只展示 `category: life` 的文章。
- `/tech/`：技术记录页面，只展示 `category: tech` 的文章。
- `/inspiration/`：灵感仓库页面，保存想法、设计片段和未来功能。
